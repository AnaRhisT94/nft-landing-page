...
function updateStatusText(isOwner, checking) {
  ...
  if (checking) {
    if (isOwner) {
      ...
      statusText.innerText = `You do own BALL TOKENS!! Let's see how many ${renderDots(dots)}`;
    } else {
      // statusText.innerText = `Checking to see how much you have deposited ${COLLECTION_NAME} ${renderDots(dots)}`;
      statusText.innerText = `Checking to see how much you have deposited ${renderDots(dots)}`;
    }
  } else {
    if (isOwner) {
      // statusText.innerText = `You own ${editions.length} ${COLLECTION_NAME}!! `;
      statusText.innerText = `You own X Tokens!! `;
    } else {
      // statusText.innerText = `You don't own any ${COLLECTION_NAME} `;
      statusText.innerText = `You don't own any tokens `;
    }
  }
